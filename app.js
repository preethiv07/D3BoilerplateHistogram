// Created by Preethi
// What : Histogram Boilerplate
// Data : Weather data for NY for 1 year
...
async function draw() {
    ...
    //container width = image width -2(margins)
    ...
    // To compare array generetad by original dataset and array of array using bin
    ...
    function histogram(metric) {
        ...
        const yAccessor = d => d.length //how many times the object is occuring. since bin is array or array, length of array will give us
        ...
        // Draw Bars 
        //x0 is minimum range for each bin and x1 is max range for an individual bin
        ctr.selectAll('rect')
            .data(newDataset)
            .join(
                ...
                .transition(exitTransition) //it should be placed before the property that needs annimation just after we join data
                // .attr('y', dimensions.ctrHeight)
                .attr('height', 0)
                .remove()
            )
            .transition(updateTransition)
            .attr('width', d => d3.max([0, xScale(d.x1) - xScale(d.x0) - padding])) //If lower than zero, use 0.assuming all bins are same size
